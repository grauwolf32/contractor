...
advance while that phase has errors. Declare kinds/tags before their first use.
For a rejected edit or repeated diagnostic, inspect its cause before another
mutation; if available evidence or tooling cannot resolve it, report the blocker
instead of cycling. Use the selected `likec4` Agent Skill when you
...
Trace important inbound and outbound flows across actual runtime units. Include
data carried across a boundary when evidenced, without copying secret values.
A dependency declaration alone does not prove a deployed service or interaction.
