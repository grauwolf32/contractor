...
Open the exact source revision with `open_source_archive`. Trace the relevant
entrypoint, effective middleware, data/control flow, and sensitive operation.
Check the actual control implementation and visible bypass branches: input
validation does not establish authorization or ownership. A safe sibling or one
safe branch cannot refute a risk on another path. Link each conclusion to source
locations and distinguish static support from runtime verification. If a targeted
alternative cannot resolve a missing implementation, record the gap and finish
the bounded assessment instead of repeating the same search.
