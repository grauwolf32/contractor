...
func (s *uiStack) runManagedEvals(owner, uiURL, apiURL, uiDirect, apiDirect string) {
	t := s.t
	...
	runChecked(t, filepath.Join(s.repositoryRoot, "ui"), environment,
		"corepack", "pnpm", "exec", "playwright", "test", "e2e/evals-stack.spec.ts", "--output", filepath.Join(output, "native"))
	runChecked(t, s.repositoryRoot, producerEnv, "python3", "-I", "-S", producer,
		"external", "--api", s.serverInternalURL, "--directory", s.temporaryRoot)
	// The same UI reads the producer's results; it has no producer command route.
	environment = append(environment, "CONTRACTOR_EVAL_EXTERNAL_ONLY=1")
	runChecked(t, filepath.Join(s.repositoryRoot, "ui"), environment,
		"corepack", "pnpm", "exec", "playwright", "test", "e2e/evals-stack.spec.ts", "--output", filepath.Join(output, "external"))
	s.assertManagedEvalExecutions(owner)
	...
	if path := os.Getenv("CONTRACTOR_EVAL_EVIDENCE_DIR"); path != "" {
		...
		for _, name := range []string{"native-evidence.json", "external-evidence.json"} {
	...
	t.Log("native Workflow/Audit browser journeys and independent stdlib external Workflow/Audit passed")
}
...
func (s *uiStack) assertManagedEvalExecutions(owner string) {
	s.t.Helper()
...
WHERE e.owner_id=$1 GROUP BY e.experiment_id ORDER BY e.control_mode,min(sub.execution_kind)`, owner)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&mode, &kind, &expected, &members, &executions); err != nil {
			s.t.Fatal(err)
		}
		if expected != 8 || members != expected || executions != expected {
			s.t.Fatalf("%s/%s matrix expected/members/ordinary executions = %d/%d/%d", mode, kind, expected, members, executions)
		}
		matrix = append(matrix, mode+"/"+kind)
	}
	if err = rows.Err(); err != nil {
		s.t.Fatal(err)
	}
	if len(matrix) != 4 {
		s.t.Fatalf("managed matrix %v, want four independent experiments", matrix)
	}
