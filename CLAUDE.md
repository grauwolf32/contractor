...
Contractor is a CLI that drives Google-ADK + LiteLLM agents to generate, enrich, and trace OpenAPI specs from a project's source code. It is **not** an HTTP service — every "run" is a CLI invocation that spins up a pipeline of agents over a target codebase and writes artifacts to a local directory.
...
`--model` is **not** a model name — it is an alias from `deploy/litellm/litellm_config.yaml` resolved by the LiteLLM proxy. Start the proxy (`cd deploy/litellm && bash run.sh`, requires Podman) before running any pipeline. Configuration is read from `cli/.env` via `pydantic-settings`; the full schema lives in `contractor/utils/settings.py` (`Settings`). Don't add config knobs anywhere else.

## Architecture cheatsheet

```
cli/main.py                       — Click entrypoint; wires flags into PipelineContext
cli/pipelines/<mode>.py           — thin assemblers: build TaskInvocations, hand them to TaskRunner
contractor/runners/task_runner.py — per-task state machine: render → spawn planner → iterate → publish
contractor/runners/agent_runner.py— bare single-agent runner used by RouterPipeline
...
- **Pipelines never call agents directly.** They build `TaskInvocation`s with a `worker_builder` partial and let `TaskRunner` spawn a fresh planner+worker pair per attempt. Use this pattern when adding a new pipeline.
