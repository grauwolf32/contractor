...
- **`iterations` vs `max_attempts`.** A task is only marked finished after `iterations` *successful* runs in total (cumulative across attempts — a failure does not reset the count); failures keep retrying until `max_attempts` is exhausted (then `TaskNotCompletedError`). Defaults come from the template's `iterations:` field.
