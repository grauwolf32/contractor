...
func TestExecutionConfigProfilePinsResolvedStageVariant(t *testing.T) {
  ...
  "stages": {
	...
	runWorkflow, err := snapshot.ResolveRunWorkflow(
		t.Context(), "openapi-from-workspace@4", patch, developmentCredentialLookup(t, snapshot),
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestExecutionConfigProfileAndRunSnapshotSurviveCatalogChange(t *testing.T) {
	...
	stored, err := first.ResolveRunWorkflow(
		t.Context(), "openapi-from-workspace@4", ExecutionConfigPatch{}, developmentCredentialLookup(t, first),
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	newWorkflow, err := second.ResolveRunWorkflow(
		t.Context(), "openapi-from-workspace@4", ExecutionConfigPatch{}, developmentCredentialLookup(t, second),
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestInlineEscalationResolvesWithoutProfileRef(t *testing.T) {
	...
	workflow, err := snapshot.ResolveRunWorkflow(
		t.Context(), "openapi-from-workspace@4", ExecutionConfigPatch{}, developmentCredentialLookup(t, snapshot),
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRunCreationValidatesCredentialsInEveryEscalationVariant(t *testing.T) {
	...
	_, err := snapshot.ResolveRunWorkflow(
		t.Context(), "openapi-from-workspace@4", ExecutionConfigPatch{}, lookup,
	)
...
func replaceEscalation(t *testing.T, root, replacement string) {
	t.Helper()
	path := filepath.Join(root, "workflows/openapi_from_workspace_v4.yaml")
