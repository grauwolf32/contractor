...
func TestRepositoryCodeAnalysisTemplateHasExactGraphToolSurface(t *testing.T) {
	...
	graph, err := snapshot.AgentTemplate("workspace_source_graph_analyst@3")
	if err != nil {
		t.Fatal(err)
	}
	assertDigest(t, graph.Ref.Digest)
	assertDigest(t, graph.Instructions.Digest)
	if graph.Instructions.Ref != "instructions/workspace-source-graph-analyst-worker-memory.md" {
...
func TestRepositoryCurrentProjectWorkflowsSelectGraphAnalysis(t *testing.T) {
	...
	for _, selector := range []string{
		"openapi-from-workspace@7",
		"openapi-from-workspace-streamline@2",
		"likec4-from-workspace@7",
		"likec4-from-workspace-streamline@4",
	} {
		...
		for _, stageName := range []string{"dependency_discovery", "project_discovery"} {
			...
			if got := binding.Template.Ref.TemplateID + "@" + binding.Template.Ref.Version; got != "workspace_source_graph_analyst@3" {
