...
func Failure(code string) *Error {
	...
	switch code {
	case "eval_invalid":
		e.Status, e.Message, e.Recovery = 422, "Invalid evaluation document.", "edit_draft"
	case "eval_limit_exceeded":
		e.Status, e.Message, e.Recovery = 422, "Evaluation document exceeds format bounds.", "edit_draft"
	case "eval_not_found":
		e.Status, e.Message, e.Recovery = 404, "Evaluation resource not found.", "none"
	case "eval_precondition_required":
		e.Status, e.Message = 428, "An evaluation revision is required."
	case "eval_revision_mismatch":
		e.Status, e.Message = 412, "The evaluation revision has changed."
