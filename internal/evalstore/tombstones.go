...
SELECT t.execution_kind, t.execution_id, t.terminal_state, t.never_started
FROM eval_execution_tombstones t
JOIN eval_experiments e USING(experiment_id)
WHERE e.owner_id=$1
    AND e.experiment_id=$2
    AND t.member_id=$3
`, owner, id, member).Scan(&t.Kind, &t.ID, &t.TerminalState, &t.NeverStarted)
