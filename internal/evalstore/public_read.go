...
func (s *Store) ScopeForMutation(ctx context.Context, owner, id, operation, key string) (Scope, error) {
	...
	err := s.db.QueryRow(ctx, `SELECT project_id FROM eval_experiments WHERE owner_id=$1 AND experiment_id=$2
 UNION SELECT project_id FROM eval_mutation_receipts WHERE owner_id=$1 AND resource_id=$5 AND operation=$3 AND operation_key=$4 LIMIT 1`, owner, lookupID, operation, key, id).Scan(&project)
...
func (s *Store) CommandRecord(ctx context.Context, owner, id, command string) (CommandRecord, error) {
	var c CommandRecord
	err := s.db.QueryRow(ctx, `SELECT c.command_id,c.experiment_id,c.kind,c.state,c.accepted_revision,c.diagnostic,c.created_at,c.finished_at FROM eval_commands c JOIN eval_experiments e USING(experiment_id) WHERE e.owner_id=$1 AND e.experiment_id=$2 AND c.command_id=$3`, owner, id, command).Scan(&c.ID, &c.ExperimentID, &c.Kind, &c.State, &c.Revision, &c.Diagnostic, &c.CreatedAt, &c.FinishedAt)
...
func (s *Store) DatasetPage(ctx context.Context, scope Scope, afterID, afterRevision string, limit int, revision *int64) (DatasetPage, error) {
	...
	var raw []byte
	err := s.db.QueryRow(ctx, `SELECT COALESCE((SELECT revision FROM eval_collections WHERE owner_id=$1 AND project_id=$2),0),COALESCE((SELECT jsonb_agg(metadata ORDER BY dataset_id,revision) FROM (SELECT dataset_id,revision,metadata FROM eval_dataset_revisions WHERE owner_id=$1 AND project_id=$2 AND (dataset_id,revision)>($3,$4) ORDER BY dataset_id,revision LIMIT $5) page),'[]'::jsonb)`, scope.OwnerID, scope.ProjectID, afterID, afterRevision, limit+1).Scan(&result.Revision, &raw)
	if err != nil {
		return result, err
	}
