...
export function AllocationListRoute() {
  ...
  return (
    <div className="panel operations-library">
      <div className="section-heading">
        <div>
          ...
          <p className="muted-copy">
            Allocated means the temporary Worker role was prepared; it does not
            prove that an A2A model call is executing now.
          </p>
        </div>
      ...
      {snapshot.allocations.length === 0 ? (
        ...
        <div className="allocation-card-list">
          {snapshot.allocations.map((allocation) => (
            <details className="allocation-card" key={allocation.allocationId}>
              <summary>
                <span>
              ...
              <div className="allocation-card-body">
                ...
                <div className="attempt-block">
                  ...
                  {!allocation.metrics.reportsComplete ? (
                    <p className="muted-copy">
                      Runtime has not submitted its final report; zero counters
                      are incomplete observations, not proof of no work.
                    </p>
                  ) : null}
                </div>
              </div>
      ...
      <p className="muted-copy">
        Lifecycle actions are Scheduler-owned. Operations cannot finish, abort,
        release, or reassign an allocation.
      </p>
    </div>
  );
}
