...
После проверки пользователем добавлена V58-013, ветка `fix/ui-finding-run-history`: описания Finding показываются полностью в Markdown; ссылки на source берутся из точных входов Audit; раскрытые labels и allocation metrics занимают ширину строки; для корзины резервируется место с внутренними отступами. Отдельной структурированной ссылки на файл/строку в Finding нет, поэтому UI не выводит её из сгенерированного описания. Задача включает повторную проверку затронутых экранов и выпуск UI 0.3.1.
