...
For `local + direct`, the disk-authoritative rule in
...
the same narrow reader and adds neither host paths nor a refresh tool.
...
The portable implementation pins exact `tree-sitter` and
`tree-sitter-language-pack` versions in the Runtime lock. It supports the
retained v1 language names from `contractor-old`:
