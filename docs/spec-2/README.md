...
This directory is the current design track for Contractor v2. It intentionally
captures the smaller execution model from the current design discussion;
requirements in the earlier [`../spec`](../spec/README.md) candidate apply here
only when they are restated in these documents.
...
[`core-execution-model.md`](core-execution-model.md) is a short compatibility
entry point for the former monolithic working agreement.
...
[05](05-first-slice-and-open-decisions.md) rather than being filled in from the
older candidate architecture.
