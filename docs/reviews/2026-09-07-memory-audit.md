...
После аудита были созданы отдельные задачи со статусом pending:
...
проблему конфигурации. Создание задач само по себе не включало Memory
в работающем приложении. Результат последующего исправления приведён ниже.

Исправления выполнены последовательно по запросу пользователя:

- V48-001: неоднозначный HTTP 5xx после Memory PUT проходит точный повтор
  и сверку результата; повторный append не дублирует подтверждённую запись.
  Коммит `08653eb05694b6533ca6d54791a08ddcec0fc53a`.
- V48-002: перечисление фильтрует `memory.` в хранилище и ограничивается
  129 bindings; 7000 обычных артефактов больше не блокируют доступ к заметкам.
  Коммит `40ac02fd5b1a294c0ff8fca0117dc551862f1e2d`.
- V48-003: полные заметки и previews сериализуют пустые теги как `[]`
  в обеих реализациях, с общей JSON-фикстурой.
  Коммит `84190d6e89460f8ddc237cbf9f1c86ff263d3419`.
- V48-004: все 20 ролей получили новые версии с шестью Memory operations
  и инструкциями; новые версии 16 Workflow и 5 AuditProfile выбирают их.
  Точные селекторы приведены в [рабочем каталоге](../../configs/MEMORY.md),
  implementation commit записан в задаче V48-004. Старые версии сохранены
  для совместимости, исходные байты эксперимента инструкций зафиксированы.

После исправлений прошли Go/Python проверки каждой задачи, PostgreSQL-тесты
без пропуска и оба сценарных E2E с настоящими процессами: прежние Memory
фикстуры и рабочий `artifact_builder` с обычным выходным артефактом.
Проверены retry, поздний Stage, изоляция namespace, пустая память нового Run
и отсутствие содержимого заметок в проверяемой диагностике.
Загрузка новых версий в уже работающий сервер в эту задачу не входила.
