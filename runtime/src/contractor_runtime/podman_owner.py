...
Runtime owns two inherited seqpacket endpoints. Health is read independently
...
async def serve_owner(
    ...
    try:
        while not lost.is_set() and not backend.closed:
            reader = asyncio.create_task(receive(control))
            ...
            try:
                ...
                break
            await asyncio.get_running_loop().sock_sendall(control, encode(response))
