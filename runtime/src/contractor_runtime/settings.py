...
from contractor_runtime.toolsets.common.target_policy import IPNetwork, parse_private_networks
...
class Settings:
    ...
    private_target_networks: tuple[IPNetwork, ...] = ()
    host: str = "127.0.0.1"
    port: int = 9443
...
def parse_settings(
    ...
    parser.add_argument("--private-target-network", action="append", default=None)
    parser.add_argument(
        "--workspace-storage",
    ...
    private_target_networks = _private_target_networks(parser, args.private_target_network, values)
    ...
    return Settings(
        ...
        workspace=workspace,
        podman=podman,
        private_target_networks=private_target_networks,
        host=host,
        port=port,
...
def _private_target_networks(
    parser: argparse.ArgumentParser,
    cli_values: list[str] | None,
    environ: Mapping[str, str],
) -> tuple[IPNetwork, ...]:
    """Operator CIDRs that model-selected HTTP and scanner targets may reach.

    Loopback, link-local and private destinations are otherwise denied unless
    they are the allocation's project HTTP target. Runtime service endpoints and
    cloud metadata addresses stay denied even inside these networks.
    """

    if cli_values is None:
        raw = environ.get("CONTRACTOR_PRIVATE_TARGET_NETWORKS", "")
        candidates = [] if raw == "" else raw.split(",")
    else:
        candidates = cli_values
    try:
        return parse_private_networks(candidates)
    except ValueError as error:
        parser.error(f"--private-target-network: {error}")
