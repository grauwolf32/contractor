...
class FindingTool:
    ...
    async def __call__(
        ...
        metric_arguments = {
            ...
            if isinstance(client_key, str) and IDENTIFIER.fullmatch(client_key)
            else "invalid",
...
def _build_request(
    *,
    ...
    _text("hypothesis", "" if hypothesis is None else hypothesis, required=False)
    ...
    if not isinstance(subject, dict) or set(subject) != {"kind", "key"}:
        raise ToolInputError("subject requires only kind and key identifiers")
    ...
    severity = "" if severity_suggestion is None else severity_suggestion
    if not isinstance(severity, str) or (severity not in SEVERITIES and severity != ""):
        raise ToolInputError(
            "severity_suggestion must be informational, low, medium, high or critical; "
            "omit it when no severity is proposed"
        )
...
def _text_size(value: object) -> int:
    return len(value.encode("utf-8", errors="replace")) if isinstance(value, str) else 0
