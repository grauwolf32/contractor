---
name: likec4
description: "Build, validate, and repair LikeC4 architecture models; use for .c4 DSL, views, deployment, predicates, styling, configuration, and CLI semantics."
compatibility: "Contractor adk@1 with likec4@1 tools"
metadata:
...
1. **Projects** - standalone LikeC4 can have multiple projects in a workspace; a project is determined by a config file (`.likec4rc`, `likec4.config.{ts,js,json}`). Contractor's `likec4@1` tools instead operate on one self-contained artifact and create an isolated temporary project for validation.
...
- In Contractor, validate only by calling `validate_likec4()` after loading or writing the current document. Do not pass a path and do not invoke a command directly.
- Internally, the runtime uses the `likec4 validate` command family (never `check`, `lint`, or `build`) with fixed `--json`, `--no-layout`, and `--file` arguments.
- `likec4 export` and its `--outdir` / `-o` flags are useful standalone CLI knowledge, but export is not exposed by the `likec4@1` Toolset.
...
1. In Contractor, load or create the one durable LikeC4 document named by the Worker instructions. Do not create project files or use filesystem paths.
2. (Required) Find existing or create a `specification { ... }` block in that document; this enables what kinds of elements/deployments/relationships/tags you can use. See Specification section below.
...
6. After each persisted phase, call `validate_likec4()` and repair reported issues.
...
- correct no-argument `validate_likec4()` call when operating inside Contractor
...
You do not invoke the LikeC4 CLI directly. First establish the current document
with `load_likec4` or `write_likec4`, persist every edit through a LikeC4 tool,
then call `validate_likec4()` with no arguments. The tool validates that exact
current artifact in an isolated single-file temporary project. There are no
unsaved filesystem edits and no caller-selected paths.

```text
validate_likec4()
```

The runtime invokes the installed `likec4` executable as follows; this is an
implementation detail, not a command for the Worker to execute:

```text
likec4 validate --json --no-layout --file <temporary-main.c4> <temporary-project-dir>
```

The fixed flags mean:

- `--json` — structured CLI output; the tool sanitizes it into the result below.
- `--no-layout` — skip layout drift checks (faster, only syntax+semantic).
- `--file <temporary-main.c4>` — validates the one current artifact.
- `<temporary-project-dir>` — an isolated runtime-owned directory.
...
Treat success as `valid: true` with `validatorAvailable: true`,
`validatorExecutionError: null`, and no issues. `valid: false` with an empty
`issues` list is still failure when the executable is unavailable, execution
failed, output was invalid, or diagnostics were truncated. The tool bounds and
sanitizes diagnostics; it rewrites diagnostic file names to `main.c4`. The
optional `stats` object is passed through only as bounded scalar fields and must
not be used to infer that other project files were validated.
...
For standalone LikeC4 usage, a config file (`likec4.config.json`, `.likec4rc`, or `likec4.config.{ts,js}`) defines a project. Contractor's `likec4@1` validation does not expose or consume a caller-managed config file; keep the Worker result self-contained.
...
Key reminders: definitions are global within the model and duplicate kind/tag identifiers cause a validation error. In Contractor, keep the specification in the same self-contained artifact as the model and views.
...
- **On a relationship** (kind spec, `with { }`, or per-view rule; v1.57.0+) — splits merged edges so each relationship between the same two elements is drawn as its own edge with its own label, instead of being collapsed into one `[...]` edge. See `references/views.md` → "Splitting Merged Relationship Edges".
...
### "I need to organize across files" (standalone LikeC4 only)

The following is general DSL knowledge. Contractor's `likec4@1` Toolset does
not expose a multi-file project, so do not use this layout for a Worker result.
...
When a model errors or the DSL seems wrong, call `load_skill_resource(skill_name="likec4", file_path="references/troubleshooting.md")`; it contains:
...
- **Validation** — distinguishing DSL issues from unavailable/failed/truncated validation
- **5-step debugging workflow** — validation-first, FQN integrity, predicate isolation, spec-first validation
- **Skill Best Practices** — response discipline, relationship disambiguation, FQN usage, safe editing rules

## Reference Index

Load a reference only when its topic is needed by calling `load_skill_resource(skill_name="likec4", file_path="references/<topic>.md")`. `SKILL.md` itself is disclosed only after `load_skill`; references remain on demand.
...
| `references/cli.md`                          | Exact no-argument `validate_likec4` contract, fixed CLI invocation, result shape, and failure semantics |
