---
description: Finding shape A/B/C/D taxonomy (taint→sink, missing control, at-rest/response exposure, business-logic invariant break), required fields, severity/confidence heuristics, and generic finding-record mapping for the trace skill. Each finding must match exactly one shape. Reaching a sink alone is NOT a vulnerability.
---
...
checklist (see `references/controls.md`).
...
## Confidence heuristics
  ...
  high   — every line in `evidence_lines` is on the actually-traced
           path; sink is labeled exactly per `references/sinks.md`;
...
## Mapping §7 fields to a finding record

When a finding-reporting tool is available, map
these fields to its closest equivalents: `name`, `place_type`, `place`,
`title`, `summary`, `severity`, `confidence`, `details`. Otherwise preserve the
same shape in the requested result artifact or response.
  ...
  details     — markdown; pack the §7 structured fields here:
                  - **CWE ID** (e.g. CWE-89 for SQL injection; load
                    `references/cwe-mapping.md` reference if unsure)
