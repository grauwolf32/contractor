---
name: vulns
description: "Conduct explicitly authorized, non-destructive web testing for IDOR, SSRF, SSTI, and XXE with evidence and remediation discipline."
compatibility: "Contractor adk@1 native Agent Skill disclosure; requires explicitly configured authorized HTTP testing facilities"
metadata:
...
This `SKILL.md` is loaded only after `load_skill(skill_name="vulns")`.
References are not injected automatically. When a class is relevant, call
`load_skill_resource(skill_name="vulns", file_path="references/<class>.md")`.
Never assume that an HTTP client, intercepting proxy, OOB listener, or other
testing facility exists unless it is exposed in the current Worker invocation.
