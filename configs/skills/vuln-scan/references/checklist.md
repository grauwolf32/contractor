...
| invariants | present / absent / weak / N/A | For money/quantity/state/workflow ops: atomic, idempotent, amounts bounded, values server-authoritative, steps ordered? (see `references/business-logic.md`) |
...
- Operations that move money / change a balance, quantity, quota, credit,
  ...
  control is present — see `references/business-logic.md`)
- Any handler that returns, logs, or persists data (run the secrets &
  sensitive-data exposure sweep — see `references/secrets.md`)
