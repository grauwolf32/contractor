...
## PHP / WordPress (see `references/php-wordpress.md` reference for full detail)
...
1. Inventory source files matching `*.py`, `*.java`, `*.go`, `*.js`, `*.ts`, `*.php`, `*.rb`, and `*.cs`
2. Grep CRITICAL patterns first (highest value per time)
3. For each hit file, inventory declarations to understand structure
