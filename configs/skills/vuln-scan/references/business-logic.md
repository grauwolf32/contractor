---
description: Business-logic / abuse-of-functionality detection for the vuln-scan skill. Flaws where every per-endpoint control (auth, authz, validation) is present but the operation still violates an application invariant. Scanners miss these because nothing matches a dangerous-sink grep — you must reason about state and sequence.
---
