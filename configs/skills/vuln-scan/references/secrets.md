---
description: Secrets & sensitive-data exposure for the vuln-scan skill. Two halves — (1) secrets at rest in code/repo (mostly covered by grep), and (2) the runtime exposure sweep (secrets/credentials/PII returned, logged, or shipped to clients) which scanners miss because nothing matches a sink grep. Run the sweep on every handler that returns, logs, or persists data.
---
...
- **Hardcoded provider keys** — high-precision regexes (AWS/GitHub/GCP/Slack/
  Stripe/OpenAI/Anthropic/JWT/private-key): see `references/miss-patterns.md` §9 and
  `references/grep-patterns.md` "Hardcoded Secrets". Any hit is almost certainly live.
- **Weak/guessable secrets** — `SECRET_KEY="secret"|"changeme"|"dev"`, short
  JWT signing keys: `references/grep-patterns.md` "Hardcoded Secrets".
- **Sensitive files committed to the repo** (CWE-538/CWE-312) — `.env*`,
  `*.pem`, `*.key`, `*.sql`, `*.bak`, `wp-config.php`, `.aws/credentials`,
  …: see `references/miss-patterns.md` §11 and the index workflow step 3.
- **Plaintext credential storage** (CWE-256) — passwords without bcrypt/
  argon2/scrypt; `==` comparison: see `references/miss-patterns.md` §4.
...
(`random.random`, `Math.random`, `mt_rand`, time-based). See `references/grep-patterns.md`
