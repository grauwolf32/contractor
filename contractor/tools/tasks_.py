...
class TaskManager:
    ...
    def save_to_state(self, ctx: Ctx) -> None:
        ...
        with open("debug.log", "a") as f:
            f.write(cls_key + "\n")
            f.write(json.dumps(cls_data[state_key]) + "\n")
            f.write(json.dumps(ctx.state[cls_key]) + "\n\n")
...
def manager_tools(name: str, max_tasks: int, _format: str = "json") -> list[Callable]:
    ...
    def advance(tool_context: ToolContext) -> dict[str, Any]:
        ...
        Reminder:
        - Call this tool AFTER worker finish the current task.
        - If status is incomplete, you MUST decompose the task before continuing.
        - This tool updates state and moves the current pointer.
        - You can review result of the current_task
        """
    ...
    def decompose_subtask(
        ...
        Args:
            subtask_id (str): id of the current subtask
            new_subtasks: (TaskDecompositionList)
        """
        mgr = _load(tool_context)
        ...
        if isinstance(new_subtasks, str):
            schema = json.dumps(TaskDecompositionList.model_json_schema())
            return {
                "error": f"new_subtasks should be formatted as TaskDecompositionList: {schema}"
            }
...
def worker_tools(name: str, max_tasks: int, _format: str = "json") -> list[Callable]:
    ...
    def report(task_result: TaskResult, tool_context: ToolContext) -> dict[str, Any]:
        ...
        Args:
            task_result (TaskResult): result description with status and execution summary
        """
        mgr = _load(tool_context)
        ...
        if isinstance(task_result, str):
            schema = json.dumps(TaskResult.model_json_schema())
            return {"error": f"task_result should be formatted as TaskResult: {schema}"}
