...
def task_tools(
    ...
    *,
    ...
    def list_subtasks(tool_context: ToolContext) -> dict[str, Any]:
        """List all subtasks with their current status.

        Returns:
            Ordered list of all subtasks.
        """
    ...
    def skip(
        ...
        IMPORTANT CONSTRAINTS:
            ...
            - For 'incomplete' subtasks: you CANNOT skip — you MUST decompose.
        """
        if not reason.strip():
        ...
        # Explicit guard: incomplete must be decomposed, never skipped
        if current.status == "incomplete":
            return {
                "error": (
                    f"Subtask `{task_id}` has status 'incomplete' and CANNOT be "
                    "skipped. Incomplete subtasks MUST be decomposed using "
                    "`decompose_subtask`. The subtask contains partial results "
