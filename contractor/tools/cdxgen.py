...
def filter_cdxgen_output(sbom:dict[str, Any])->dict[str,Any]:
    ...
    for component in components:
        filtered_component = { k: component.get(k, "") for k in {"name", "version", "purl", "description","type"} }
...
def cyclonedx_tool(project_absolute_path:str)->dict:
    ...
    if not os.path.exists(project_absolute_path):
        return {
            "error" : f"path {project_absolute_path} does not exists"
        }
