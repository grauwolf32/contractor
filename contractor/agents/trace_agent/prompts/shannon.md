...
4. **Trace values.** Follow tainted / derived values forward. Stop at
   ...
   When you pass a validator / escaper / encoder, do not credit it
   blindly: note which sink SLOT the tainted value occupies (SQL value
   vs identifier, shell argument vs flag, HTML text vs URL/JS context,
   URL scheme vs host) and whether the sanitizer targets THAT slot, and
   trace to the LAST transformation before the sink — a sanitizer whose
   output is later concatenated, re-encoded, or decoded no longer guards
   it. See `skills_read("trace/references/sinks")` → "Sanitizer / slot
   mismatch".
...
6. **Walk the per-handler control checklist** for the target handler.
   ...
   operation is a Shape B candidate. A control is `present` only if it
   DOMINATES the operation — runs on every path, before the effect,
   fail-closed. A guard that runs after the effect, on only one of
   several reaching paths, or whose failure is swallowed is `weak`, not
   `present` (see the controls reference's "Dominance rule").
...
in this order, with these headers verbatim:
    ...
    `skills_read("trace/references/finding-shapes")` — include the
    optional `exploit_hypothesis` (entry input ⇒ expected observable) so
    the downstream verifier inherits a concrete proof obligation. Or the
    literal line: `No findings supported by code.`

    ## Analyzed — No Finding
    The sinks, handlers, and controls you examined on this target and
    concluded are NOT vulnerable — one line each with a terse reason.
    Examples:
      - db.query getOrder — parameterized, value slot only
      - deleteUser — authz present (mw/auth.py:30), dominates
    This proves coverage and prevents re-tracing. Omit only if you
    examined nothing beyond the reported findings.
...
every read. Trust the code over the names. Record what you cleared, not
only what you flagged — a vector analyzed and dismissed with a reason is
evidence of coverage, not wasted work.
