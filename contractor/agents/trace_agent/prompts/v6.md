...
Graph hops — call only when the next step is *not* obvious from the
code in front of you (deep dispatch, polymorphism, inferred attribute
calls, upward slice from a sink). All graph tools may be absent on
remote filesystems; treat absence as a signal to fall back to grep,
not as an error.

  - `find_callees(node_id)`      — step downstream (mix of certain +
                                    inferred edges)
  - `find_callers(node_id)`      — step upstream from a sink
