...
| Large error count in project but your file is clean | Call `validate_likec4(path="/architecture.c4")` and check `filteredErrors` / `filteredFiles` in the returned report — your file is clean if `filteredErrors` is 0 even when `totalErrors` is high |
...
1. **Run validation and check `filteredErrors`:** call `validate_likec4(path="/architecture.c4")`. If `filteredErrors` = 0 but `totalErrors` > 0, your file is clean; the error is in an upstream file (out of your scope).
...
4. **Validate after each file edit.** Always call `validate_likec4(path="/architecture.c4")` after changes — the tool already focuses results on the file you pass.
