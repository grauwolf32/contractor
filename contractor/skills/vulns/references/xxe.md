---
name: xxe
description: XXE testing checklist — classic & blind (OOB), error-based, XInclude, file upload (SVG/DOCX/EPUB/XLSX), SOAP/SAML, K8s admission webhooks, CI/CD XML artifacts, OOB exfil via external DTD, PHP `php://filter` base64, parser-hardening snippets per language.
---

# XML External Entity (XXE) Injection

Trigger signals: `application/xml` / `text/xml` content type, SOAP envelope, SAML AuthnRequest/Response, file uploads accepting DOCX/SVG/EPUB/XLSX/PDF/ZIP, JSON→XML conversion, RSS feed parsers.
