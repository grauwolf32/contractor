---
name: ssti
description: SSTI testing checklist — engine fingerprint via polyglot (Jinja2, Twig, FreeMarker, Pebble, Velocity, Mako, Thymeleaf, Razor, Liquid, Nunjucks, Handlebars, Blade, Groovy/GSP, ERB), engine-specific RCE payloads, sandbox escape, prototype-chain RCE in Node templates, blind SSTI, filter bypass, recent CVEs.
---

# Server-Side Template Injection (SSTI)

Trigger signals: reflected input that *evaluates* server-side (`{{7*7}}` → `49`, `${7*7}` → `49`, `@(1+2)` → `3`), template engine names in stack traces, debug variables (`{{config}}`, `{$smarty}`).
